...
import xyz.acrylicstyle.anymixin.AnyMixin;
import xyz.acrylicstyle.anymixin.util.ClasspathUtil;
...
public class Launch {
    ...
    private Launch() {
        ...
        if (cl instanceof URLClassLoader) {
            classLoader = LaunchClassLoader.createClassLoaderWithParent(((URLClassLoader) cl).getURLs(), null);
        } else {
            Set<URL> cp = new HashSet<>(Arrays.asList(tryGetURLsFromAppClassLoader(cl)));
            try {
                if (cp.isEmpty()) {
                    cp.add(ClasspathUtil.getClasspathAsURL(AnyMixin.class));
                }
            ...
            if (modulesDir.isDirectory()) {
                for (File file : lookForPlugins(modulesDir)) {
                    try {
    ...
    @NotNull
    private static Deque<File> lookForPlugins(@NotNull File pluginsDir) {
        LOGGER.info("Looking for mods in " + pluginsDir.getAbsolutePath());
